...
$(document).ready(function(){
	...
	$('#savesearch').click(function(){
		...
		$.ajax({
		   	...
		   	type: 'POST',
		   	data: {'action': 'save_area_session', 'area': area},
	...
	function showPosition(position) {
		...
		jQuery.ajax({
			...
			success: function (data) {
				...
				//    url += '?area='+data
				// }
				url = updateQueryStringParameter(url, 'area', data);
				// param += updateQueryStringParameter(url, 'latitude', latitude);
				// param += updateQueryStringParameter(url, 'longitude', longitude);
				// url = param;
...
function sendMSG(mobileNumber,msg){
	//console.log(country_code +mobileNumber);
	console.log(msg);
	...
	jQuery.ajax({
       url: ajax_object.ajax_url,
       type: 'POST',
       data: {'action': 'sendMSG', 'mobile_number': mobileNumber, 'msg': msg},
       success: function (data) {
	       //console.log(data);
	       try {
		        var jsonObj = JSON.parse(data);
...
function selectArea(val) {
	$("#txtArea, #searchArea").val(val);
	$("#resultdropdown, #resultsearch").hide();
}