				...
				<script type="application/ld+json">
					{
						...
						<?php if(!empty(get_field('first_reference_review')) || !empty(get_field('second_reference_review')) || !empty(get_field('third_reference_review'))) { ?> 
						"review": [{
							"@type": "Review",
...
<div class="top_navbar">
	<div class="container">
		<div class="row">
			<div class="col-sm-6 cal-12 ml-auto text-right">
				<ul>
					<!-- <li>
						<a href=""><span class="icon-location-header"></span><span id="menu_geolocation" class="tl-txt">Mumbai</span></a>
					</li> -->
					<li class="user_name_top">
						<?php
							if(!is_user_logged_in())
							{
								?><a href="login/" class="tl-txt">Login</a><?php
							}
							else
							{
								$userPost = get_post(wp_get_current_user()->post_id);
								$therapist_profile_url = get_site_url().'/therapist/'.$userPost->post_name;
								?>
								<span class="tl-txt"><?php echo wp_get_current_user()->first_name . ' ' . wp_get_current_user()->last_name; ?></span>
									...
									<ul class="user-sub-action">
										...
										<li id="user-sub-action-dashboard">
											<?php
												...
												{
													$dashboard = site_url() . "/" ."therapist-account-dashboard/";
												}
												else if(in_array("subscriber", wp_get_current_user()->roles))
												{
												$dashboard = "my-account-page/";
												}
												else
												{
												$dashboard = site_url() . "/" ."admin-chat-dashboard/";	
												}
											?>
					...
					<li>
						<a href="<?php echo get_permalink( get_page_by_path( 'search-page' ) ); ?>" class="search-top-header"></a>	
...
-->
			...
			<div class="col-3 col-sm-2 icons-set">
				<a href="<?php echo get_permalink( get_page_by_path( 'search-page' ) ); ?>" class="right-icons icon-search"></a>	
				...
				?>
				<span class="user-loggedin"><a href="" class="link_profile"><i class="fa fa-user-circle-o" aria-hidden="true"></i></a>
				<div class="user-sub-action-wrapper">
					<ul class="user-sub-action">
						<li class="user-sub-action-user-info">
							<!-- <a class="ua-item" tabindex="-1" href="<?php echo $therapist_profile_url; ?>"> -->
						...
						<li id="user-sub-action-dashboard">
							<?php
								...
								{
									$dashboard = site_url() . "/" ."my-account-page/";
								}
							?>
							<a class="ab-item" href="<?php echo $dashboard; ?>">View Dashboard</a>
						</li>
		...
		<div class="row">
			<div class="col-12 banner-above-menu-wrapper">
				<?php
					wp_nav_menu(array(
				...
				?>
				<span href="" class="location-wrapper-top" id="openlocationbox"><span class="icon-location-header"></span><span id="menu_geolocation" class="tl-txt"><?php if($_SESSION['user_area']){echo $_SESSION['user_area'];} else {echo 'Mumbai';} ?></span></span>
			</div>
		</div>
		<div id="locationsearch" style="display: none; float: right;" class="row">
			<button class="col-md-2" type="button" id="savesearch"><i class="icon-location-header" aria-hidden="true"></i></button>
