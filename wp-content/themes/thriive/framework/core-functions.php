			...
			else
			{
				...
				if(!is_wp_error($userInfo))
				{
					...
					else if(in_array("subscriber", $userInfo->roles)) //if(($userInfo->roles[0]) == 'subscriber')
					{
						...
						else if(in_array("customer", $userInfo->roles)) //if(($userInfo->roles[0]) == 'subscriber')
					{
						auto_login_new_user($user_id);
						if(isset($input['call_consult'])){
							wp_redirect($input['call_consult']);
...
function save_area_session(){
	...
	$areaResult = getLatLng($keyword);
	print_r($areaResult); exit;
	echo json_encode($areaResult);
	wp_die();
}
	      	...
	      	if($zipcode != "" && empty($area)){
	      		...
	      		if(count($get_area_lists_by_zipcode) > 1){
		      		foreach ($get_area_lists_by_zipcode as $csv_area) {
						...
						update_field('latitude',$getLatLng[0],$post_id);
						update_field('longitude',$getLatLng[1],$post_id);
		      		}
		      	} else {
...
function getLatLng($address){
	...
	$ch = curl_init("https://maps.googleapis.com/maps/api/geocode/json?address=".$address."&key=AIzaSyCkzH5PXPkf-Er-f6EEEsRQfYSBzzFD6K4");
    ...
    curl_close($ch);
    array_push($results,$decodeData->results[0]->geometry->location->lat,$decodeData->results[0]->geometry->location->lng);
