...
function selectAreaTop(val) {
	$("#searchArea").val(val);
	$("#resultsearch").hide();
	var url = window.location.href+'/therapist';
	$.ajax({
	   	url: ajax_object.ajax_url,
	   	type: 'POST',
...
function changeRating(cid)
   	{
    	var cname=document.getElementById(cid).className;
      	var ab=document.getElementById(cid+"_hidden").value;
      	// for(var i=1;i<=ab;i++){

      	// }
      	if(cname == 'fa fa-star-o'){
        	$("#"+cid).removeClass("fa-star-o");
        	$("#"+cid).addClass("fa-star");
      	} else {
      		$("#"+cid).removeClass("fa-star");
        	$("#"+cid).addClass("fa-star-o");
      	}
      	$('#rating').val(ab);
   	}