...
function getContactedTherapist(){
	ob_start();
	?>
	<table class="table table-striped table-bordered">
		<tr>
			...
			<th>Seeker id</th>
			<th>Seeker's Name</th>
			<th>Seeker's Username</th>
			<th>Seeker's Email</th>
			<th>Therapist Name</th>
			<th>Connected Date Time</th>
			<th>Call</th>
			<th>Consult</th>
		</tr>
		<?php
			$all_subscriber = getContactedTherapistSeekers();
			$subscriber_count = 0;
			foreach($all_subscriber as $subscriber)
			{ ?>
				<tr>
					<td><?php echo ++$subscriber_count; ?></td>
					<td><?php echo $subscriber['id']; ?></td>
					<td><?php echo $subscriber['name']; ?></td>
					<td><?php echo $subscriber['uname']; ?></td>
					<td><?php echo $subscriber['email']; ?></td>
					<td><?php echo $subscriber['therapist_name']; ?></td>
					<td><?php echo $subscriber['timestamp']; ?></td>
					<td><?php echo (($subscriber['is_call']) == 1 ? "Yes" : "-" ); ?></td>
					<td><?php echo (($subscriber['is_call']) == 0 ? "Yes" : "-" ); ?></td>
				</tr>
				<?php
			}
		?>
...
function getConnectedTherapist()
{
	...
	<table class="table table-striped table-bordered">
		...
		<?php
			$all_subscriber = getAllTherapistByRole('subscriber');
			//echo "<pre>";print_r($all_subscriber);echo "</pre>";die;
			$subscriber_count = 0;
			foreach($all_subscriber as $subscriber)
			{
				$user = get_user_by( 'id', $subscriber['id'] );

				//Get connected therapist Name
				$total_contacted_therapist = get_user_meta($subscriber['id'], 'contacted_therapist_id', true);
				if(empty($total_contacted_therapist))
				{
					continue;
				}
				$total_contacted_therapist = explode(",", $total_contacted_therapist);
				$all_therapist = array();
				foreach($total_contacted_therapist as $therapist)
				{
					$first_name = get_user_meta($therapist, 'first_name', true);
					$last_name = get_user_meta($therapist, 'last_name', true);
					if(!empty($first_name) || !empty($last_name))
					{
						$therapist_name =  $first_name . " " . $last_name;
					}
					else
					{
						$therapist_name = "N/A";
					}
					$all_therapist[] = $therapist_name;
				}
				$all_therapist = implode(", ", $all_therapist);
				$contacted_therapist_date = get_user_meta($subscriber['id'],'contacted_therapist_date',true);
				$contacted_therapist_time = get_user_meta($subscriber['id'],'contacted_therapist_time',true);
				?>
				<tr>
					<td><?php echo ++$subscriber_count; ?></td>
					<td><?php echo $subscriber['id']; ?></td>
					...
					<td><?php echo $subscriber['email']; ?></td>
					<td><?php echo $all_therapist; ?></td>
					<td><?php echo $contacted_therapist_date; ?></td>
					<td><?php echo $contacted_therapist_time; ?></td>
				</tr>
				<?php
			}
		?>
...
//Users GeoLocation
function nks_getUsersGeoLocation() {
	ob_start();
	?>
	<table class="table table-striped table-bordered">
		<tr>
		...
		<?php
			global $wpdb;
			$user_results = $wpdb->get_results( 'select * from wp_user_geolocation');
			foreach($user_results as $user_result) {
				?>
				<tr>
					<td><?php echo $user_result->user_id; ?></td>
					<td><?php echo $user_result->user_name; ?></td>
					<td><?php echo $user_result->user_email; ?></td>
					<td><?php echo $user_result->location_date; ?></td>
					<td><?php echo $user_result->location_time; ?></td>
					<td><?php echo $user_result->user_ip; ?></td>
					<td><?php echo $user_result->latitude; ?></td>
					<td><?php echo $user_result->longitude; ?></td>
				</tr>
				<?php
			}
			//$user_location = getUsersGeoLocation();
...
/*
			foreach($all_subscriber as $subscriber)
			{
				...
				foreach($total_contacted_therapist as $therapist)
				{
					...
					if(!empty($first_name) || !empty($last_name))
					{
						$all_therapist[] =  $first_name . " " . $last_name;
					}
				}
				$all_therapist = implode(", ", $all_therapist);
				?>
